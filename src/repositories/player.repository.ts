...
export const PlayerRepository = {
  async getAll(connectionString: string): Promise<Array<IPlayer>> {
    try {
      ...
      const returnedData =
        await sql`SELECT id, name, country, to_char(birth_at, 'DD-MM-YYYY') AS birth_at FROM players`;
  ...
  async getById(connectionString: string, playerId: number): Promise<IPlayer> {
    try {
      ...
      const returnedData = await sql.query(
        `SELECT id, name, country, to_char(birth_at, 'DD-MM-YYYY') AS birth_at FROM players WHERE id = $1`,
        [playerId],
      );
  ...
  async addPlayer(connectionString: string, player: IPlayer): Promise<IPlayer> {
    try {
      ...
      const returnedData = await sql.query(
        `
          INSERT INTO players (name, country, birth_at) 
  ...
  async updatePlayer(
    ...
    try {
      ...
      const returnedData = await sql.query(
        `
          UPDATE players 
          SET name = $1, country = $2, birth_at = $3
          WHERE id = $4
          RETURNING *`,
